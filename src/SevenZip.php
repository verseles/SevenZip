...
class SevenZip
{
    ...
    /**
     * Cache for parsed 7-Zip info to avoid repeated external calls.
     *
     * @var array|null
     */
    ...
    protected function runCommand(array $command, bool $secondary = false): string
    {
        ...
        if (!$process->isSuccessful()) {
            $maskedCommand = $this->maskPasswordInCommand($command);
            throw new \RuntimeException(
                "Command: " . implode(" ", $maskedCommand) . "\n" .
    ...
    {
        if ($output === null && $this->parsedInfoCache !== null) {
            return $this->parsedInfoCache;
        }

        $parsed = $this->parseInfoOutput($output ?? $this->getInfo());

        if ($output === null) {
            $this->parsedInfoCache = $parsed;
        }
    ...
    public function extract(): string
    {
        ...
        if ($shouldDeleteSourceAfterExtract && $sourcePath && is_file($sourcePath)) {
            unlink($sourcePath);
        }
     ...
     *
     * @param array $renameMap An associative array where keys are the old names and values are the new names.
     *
     * @return string The output of the 7-Zip command.
     ...
     * @return $this
     */
    public function m1(string $method): self
    {
    ...
    /**
     * Mask password in command array to avoid exposing sensitive information in logs.
     *
     * @param array $command The command array to mask.
     *
     * @return array The command array with masked password.
     */
    protected function maskPasswordInCommand(array $command): array
    {
        ...
        return array_map(function ($arg) {
            if (is_string($arg) && str_starts_with($arg, '-p')) {
                return '-p' . str_repeat('*', 8);
            }
            return $arg;
        }, $command);
    }
