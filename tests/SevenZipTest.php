...
class SevenZipTest extends TestCase
{
    ...
    #[Covers('\Verseles\SevenZip\SevenZip::getProgressCallback')]
    ...
    #[Covers('\Verseles\SevenZip\SevenZip::setCustomFlags')]
    ...
    public function testRename(string $format): void
    {
        ...
        $renames = ['source/Avatart.svg' => 'source/Avatar_renamed.svg'];
    ...
    public function testExtract(string $format): void
    {
        ...
        if ($format !== 'bzip2') {
            $this->assertFileExists(filename: $target . '/source/Avatar_renamed.svg');
        } else {
            // Because bzip2 does not support folders in this test and rename failed
            // we will just assume it works for what it is, but let's check Avatart
    ...
    public function testEncryptAndDecrypt(): void
    {
        ...
        $verifyOutput = $this->sevenZip->setPassword($password)
          ->source($encryptedFile)
          ->verify();

        $this->assertStringContainsString('Everything is Ok', $verifyOutput);
